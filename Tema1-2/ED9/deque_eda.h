...
public:
	...
	// consultar si la dcola est?vacía
	bool empty() const {
		return nelems == 0;
	}
	...
	void invertir() {
		if (fantasma->ant == nullptr) {//Si la cola est?vacía
			return;
		}
		Nodo* first = fantasma->sig;
		Nodo* final = fantasma->ant;
		int i = 0;
		int m = nelems / 2;

		while (i < m) {
			int n = fantasma->sig->elem;
			fantasma->sig->elem = fantasma->ant->elem;
			fantasma->ant->elem = n;
			fantasma->sig = fantasma->sig->sig;
			fantasma->ant = fantasma->ant->ant;
			i++;
		}
		fantasma->sig = first;
		fantasma->ant = final;
