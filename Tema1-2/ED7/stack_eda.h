...
public:
	stack() : nelems(0), capacidad(TAM_INICIAL), array(new T[capacidad]) {}// constructor: pila vacía
	~stack() { libera(); } // destructor
	stack(stack<T> const& other) { copia(other); }// constructor por copia
	...
	// consultar si la pila est?vacía
	bool empty() const {
		return nelems == 0;
	}
