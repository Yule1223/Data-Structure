...
public:
	...
	// consultar el número de nodos
	size_t nNodos() const {
		return nNodos(this->raiz);
	}

	// consultar el número de hojas
	size_t nHojas() const {
...
private:
	...
	static size_t nNodos(Link r) {
		if (r == nullptr) {
			return 0;
		}
		else {
			return nNodos(r->left) + nNodos(r->right) + 1;
		}
	}

	static size_t nHojas(Link r) {
		if (r == nullptr) {
			return 0;
		}
		else {
			if (r->left == nullptr && r->right == nullptr)
				...
				return nHojas(r->left) + nHojas(r->right);
		}
	}
