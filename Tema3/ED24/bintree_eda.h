...
public:
	...
	// consultar si el árbol est?vacío
	bool empty() const {
		return raiz == nullptr;
	}
	...
	bintree<T> reconstruccion(vector<int>const& pre, int preStart, int preEnd, vector<int>const& in, int inStart, int inEnd) const {
		...
		for (int i = inStart; i <= inEnd; i++) {
			if (in[i] == pre[preStart]) {
				Iz = reconstruccion(pre, preStart + 1, preStart + i - inStart, in, inStart, i - 1);
				Dr = reconstruccion(pre, preStart + i - inStart + 1, preEnd, in, i + 1, inEnd);
			}
		}
		bintree<T> arbol = bintree(Iz, pre[preStart], Dr);
		return arbol;
	}
